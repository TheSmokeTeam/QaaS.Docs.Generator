...
internal sealed class CliReferenceRenderer
{
    public IReadOnlyList<GeneratedDocument> RenderRunner(RunnerCliCatalog catalog)
    {
        ...
        documents.AddRange(
            catalog.Commands.Select(command => new GeneratedDocument(
                $"{commandRoot}/{command.Name}.md",
                GeneratedDocumentHasher.WithHeader(
                    RenderRunnerCommand(command),
                    ["Runner", command.Name, "cli-command"]
                )
            ))
        );
    ...
    public IReadOnlyList<GeneratedDocument> RenderMocker(MockerCliCatalog catalog)
    {
        ...
        documents.AddRange(
            catalog.Commands.Select(command => new GeneratedDocument(
                $"{commandRoot}/{command.Name}.md",
                GeneratedDocumentHasher.WithHeader(
                    RenderMockerCommand(command),
                    ["Mocker", command.Name, "cli-command"]
                )
            ))
        );
    ...
    private static string RenderRunnerOverview(RunnerCliCatalog catalog)
    {
        ...
        builder.AppendLine("| ------- | ----------- | -------- |");
        foreach (
            var command in catalog.Commands.OrderBy(command => command.Name, StringComparer.Ordinal)
        )
        {
            builder.AppendLine(
                $"| [`{command.Name}`](./{command.Name}.md) | {Escape(command.Description)} | {Escape(CliCommandGuides.RunnerOverviewSummary(command.Name))} |"
            );
        }
        ...
        {
            builder.AppendLine(
                $"| {CliCommandGuides.RunnerFlagCategory(option)} | `{FormatFlag(option.ShortName, option.LongName)}` | {Escape(option.DefaultValue ?? string.Empty)} | `{TypeDisplayFormatter.FormatValueType(option.ValueType)}` | {Escape(option.HelpText ?? string.Empty)} |"
            );
        }
    ...
    private static string RenderMockerOverview(MockerCliCatalog catalog)
    {
        ...
        {
            builder.AppendLine(
                $"| [`{command.Name}`](./{command.Name}.md) | {Escape(command.Description)} | {Escape(CliCommandGuides.MockerOverviewSummary(command.Name))} |"
            );
        }
        ...
        {
            builder.AppendLine(
                $"| {CliCommandGuides.MockerFlagCategory(option)} | `{FormatFlag(option.ShortName, option.LongName)}` | {Escape(option.DefaultValue ?? string.Empty)} | `{TypeDisplayFormatter.FormatValueType(option.ValueType)}` | {Escape(option.HelpText)} |"
            );
        }
        ...
        builder.AppendLine();
        AppendCodeBlock(builder, "text", NormalizeHelpText(catalog.OverviewHelpText));
        AppendSeeAlso(builder, ["- [`run`](./run.md)", "- [`template`](./template.md)"]);
        return builder.ToString().TrimEnd();
    }
    ...
    private static string RenderRunnerCommand(RunnerCliCommand command)
    {
        ...
        builder.AppendLine();
        builder.AppendLine($"> TL;DR — {command.Description}");
        builder.AppendLine();
        builder.AppendLine("## Invocation");
        builder.AppendLine();
        AppendCodeBlock(builder, "bash", CliCommandGuides.RunnerInvocation(command.Name));
        AppendBullets(builder, "When to use", CliCommandGuides.RunnerUseWhen(command.Name));
        ...
        foreach (
            var option in command.Options.OrderBy(
        ...
        AppendSeeAlso(builder, RunnerCommandSeeAlso(command.Name));
        return builder.ToString().TrimEnd();
    }
    ...
    private static string RenderMockerCommand(MockerCliCommand command)
    {
        ...
        AppendBullets(builder, "When to use", CliCommandGuides.MockerUseWhen(command.Name));
        ...
        builder.AppendLine();
        AppendCodeBlock(builder, "text", NormalizeHelpText(command.HelpText));
        AppendSeeAlso(builder, ["- [Mocker commands](commands.md)"]);
        return builder.ToString().TrimEnd();
    }

    private static void AppendRunnerExitCodes(StringBuilder builder)
    {
        builder.AppendLine();
    ...
    private static IReadOnlyList<string> RunnerCommandSeeAlso(string commandName)
    {
        return string.Equals(commandName, "run", StringComparison.Ordinal)
            ?
            [
    ...
    private static void AppendSeeAlso(StringBuilder builder, IReadOnlyList<string> links)
    {
        builder.AppendLine();
    ...
    private static void AppendExamples(
        StringBuilder builder,
        IEnumerable<CliCommandExample> examples
    )
    {
    ...
    private static IReadOnlyList<RunnerCliArgument> GetCommonRunnerOptions(RunnerCliCatalog catalog)
    {
        return catalog
            .Commands.Select(command =>
                command.Options.Select(option => option.LongName).ToHashSet(StringComparer.Ordinal)
            )
            .Aggregate(
                (left, right) =>
                {
            ...
            .Let(commonOptions =>
                catalog
                    .Commands.SelectMany(command => command.Options)
                    .Where(option =>
                        option.LongName is not null && commonOptions.Contains(option.LongName)
                    )
    ...
    {
        var rawLines = helpText.TrimEnd().Split(["\r\n", "\n"], StringSplitOptions.None);
        ...
        for (var index = 0; index < rawLines.Length; index++)
        {
            ...
            if (
                (
                    line.Contains("CliExport", StringComparison.Ordinal)
...
{
    public static TResult Let<TValue, TResult>(this TValue value, Func<TValue, TResult> selector) =>
        selector(value);
}
