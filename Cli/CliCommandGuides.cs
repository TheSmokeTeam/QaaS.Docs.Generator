...
{
    public static string RunnerOverviewSummary(string commandName) => commandName switch
    {
        "run" => "Run the full QaaS flow: sessions, assertions, reporting, and optional Allure serving.",
    ...
    public static string MockerOverviewSummary(string commandName) => commandName switch
    {
        "run" => "Start the configured mock servers and optional controller runtime.",
        "template" => "Render the effective merged mocker configuration without starting the runtime.",
        _ => "Mocker command."
    };

    public static string RunnerInvocation(string commandName) => commandName switch
    {
    ...
    public static IEnumerable<string> RunnerUseWhen(string commandName) => commandName switch
    {
        "run" =>
        [
    ...
    public static IEnumerable<string> MockerUseWhen(string commandName) => commandName switch
    {
        "run" =>
        [
    ...
    public static IEnumerable<CliCommandExample> RunnerExamples(string commandName) => commandName switch
    {
        "run" =>
        [
            new("Run the default configuration file", "dotnet run -- run test.qaas.yaml"),
            new("Run only one case and one session", "dotnet run -- run test.qaas.yaml -c cases -n happy-path -i Checkout"),
            new("Run and open an Allure 3 style report folder", "dotnet run -- run test.qaas.yaml -s allure-report", "Pass bare `-s` to serve the default `allure-results` folder instead.")
        ],
        "act" =>
        [
            new("Run sessions and store the produced session data", "dotnet run -- act test.qaas.yaml"),
            new("Capture only a focused subset of the test", "dotnet run -- act test.qaas.yaml -c cases -n happy-path -i Checkout")
        ],
        "assert" =>
        [
            new("Assert against the stored session data for a configuration", "dotnet run -- assert test.qaas.yaml"),
            new("Assert and open the results folder", "dotnet run -- assert test.qaas.yaml -s allure-results")
        ],
        "template" =>
        [
            new("Print the resolved configuration", "dotnet run -- template test.qaas.yaml"),
            new("Preview the merged config with one extra file and one override", "dotnet run -- template test.qaas.yaml -w local.qaas.yaml -r MetaData:Environment=qa")
        ],
        "execute" =>
        [
            new("Execute every command in the YAML file", "dotnet run -- execute executable.yaml"),
            new("Execute only selected command IDs", "dotnet run -- execute executable.yaml -c smoke assert-only"),
            new("Execute the flow and open the report folder", "dotnet run -- execute executable.yaml -s allure-report")
        ],
        _ => []
    };

    public static IEnumerable<CliCommandExample> MockerExamples(string commandName) => commandName switch
    {
        "run" =>
        [
            new("Start the mocker with the default configuration", "dotnet run -- run mocker.qaas.yaml"),
            new("Start the mocker with local overrides", "dotnet run -- run mocker.qaas.yaml -w local-overrides.yaml -r Server:Port=8081"),
            new("Run locally and keep the host interactive", "dotnet run -- run mocker.qaas.yaml --run-locally")
        ],
        "template" =>
        [
            new("Print the resolved mocker configuration", "dotnet run -- template mocker.qaas.yaml"),
            new("Render the merged config into an output folder", "dotnet run -- template mocker.qaas.yaml -w local-overrides.yaml -o rendered")
        ],
        _ => []
    };

    public static IEnumerable<CliMarkdownSection> RunnerSections(string commandName) => commandName switch
    {
        "execute" =>
        [
        ...
        "run" or "act" or "assert" or "template" =>
        [
            new("Flag Notes", """
    ...
    public static IEnumerable<CliMarkdownSection> MockerSections(string commandName) => commandName switch
    {
        "run" =>
        [
            new("Flag Notes", """
    ...
    public static string RunnerFlagCategory(RunnerCliArgument option)
    {
        ...
        {
            "with-files" or "with-folders" or "overwrite-arguments" or "push-references" or "no-env" or "resolve-cases-last" => "Configuration",
            "cases" or "cases-names" or "cases-names-ignore" or "cases-name-patterns-ignore" or "session-names" or "session-categories" or "assertion-names" or "assertion-categories" or "command-ids-to-run" => "Selection",
    ...
    public static string MockerFlagCategory(MockerCliOption option)
    {
        ...
        return option.LongName switch
        {
            "overwrite-files" or "overwrite-folders" or "overwrite-arguments" or "no-env" => "Configuration",
...
internal sealed record CliCommandExample(string Title, string Code, string? Note = null, string Language = "bash");
